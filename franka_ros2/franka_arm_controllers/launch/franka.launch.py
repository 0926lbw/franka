...
# For repeatable runs, prefer wrapping this launch file in a project-specific launch file
# or passing the parameters from a script.
#
...
# It is designed to be called directly or included by higher-level project launch files.
...
def generate_robot_nodes(context):
    ...
    if os.path.isabs(urdf_file):
        urdf_path = urdf_file
    elif urdf_package == "franka_description":
        urdf_path = PathJoinSubstitution(
            [FindPackageShare(urdf_package), "robots", urdf_file]
        ).perform(context)
    else:
        urdf_path = PathJoinSubstitution([FindPackageShare(urdf_package), urdf_file]).perform(
            context
        )
...
def generate_launch_description():
    launch_args = [
        ...
        DeclareLaunchArgument(
            ...
            description="Package that contains urdf_file. franka_description paths are relative to robots/",
        ),
        DeclareLaunchArgument(
            "robot_ip",
